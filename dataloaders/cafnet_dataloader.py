...
class CaFNetDataLoader(object):
    def __init__(self, args, mode):
        if mode == 'train':
            ...
            ground_truth_nointer_paths = read_paths(self.train_ground_truth_nointer_path)
            lidar_paths = read_paths(self.train_lidar_path)
            ...
            ground_truth_nointer_paths=ground_truth_nointer_paths, lidar_paths=lidar_paths, transform=preprocessing_transforms(mode))
...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, ground_truth_paths=None, image_paths=None, radar_paths=None, box_pos_paths=None,\
                 main_path=None, ground_truth_nointer_paths=None, lidar_paths=None, transform=None, is_for_online_eval=False):
        self.args = args
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            image, depth_gt, radar_channels, box_pos, radar_gt, nointer_depth_gt, lidar = self.random_crop(image, depth_gt, radar_channels, box_pos, radar_gt, \
                                                                                  nointer_depth_gt, lidar, self.args.input_height, self.args.input_width)
            image, depth_gt, radar_channels, box_pos, radar_gt, nointer_depth_gt, lidar = self.train_preprocess(image, depth_gt, radar_channels, box_pos, radar_gt, nointer_depth_gt, lidar)
            sample = {'image': image, 'depth': depth_gt, 'focal': focal, 'radar': radar_channels, 'box_pos': box_pos, 'radar_gt': radar_gt, \
                      'nointer_depth': nointer_depth_gt, 'lidar': lidar}
    ...
    def random_crop(self, img, depth, rad, box_pos, radar_gt, nointer_depth_gt, lidar, height, width):
        ...
        nointer_depth_gt = nointer_depth_gt[y:y + height, x:x + width, :]
        lidar = lidar[y:y + height, x:x + width, :]
        return img, depth, rad, box_pos, radar_gt, nointer_depth_gt, lidar

    def train_preprocess(self, image, depth_gt, radar, box_pos, radar_gt, nointer_depth_gt, lidar):
        # Random flipping
        ...
        return image, depth_gt, radar, box_pos, radar_gt, nointer_depth_gt, lidar
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'train':
            ...
            return {'image': image, 'depth': depth, 'focal': focal, 'radar': radar, 'box_pos': box_pos, 'radar_gt': radar_gt, 'nointer_depth': nointer_depth, 'lidar': lidar}
        else:
