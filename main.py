...
def online_eval(model, dataloader_eval, gpu, ngpus):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            _, _, _, pred_depth, _, _ = model(image, radar, focal)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model_pre = CaFNet(args)
    model_pre = torch.nn.DataParallel(model_pre)
    checkpoint_path = './saved_models/CaFNet/model-435000-best_mae_2.09121'
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    global_step = checkpoint['global_step']
    model_pre.load_state_dict(checkpoint['model'])

    state_dict_model = model.module.state_dict()
    state_dict_model_pre = model_pre.module.state_dict()

    common_keys = {key: value for key, value in state_dict_model_pre.items() if key in state_dict_model}
    state_dict_model.update(common_keys)
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            # feat_T, radar_feat_T, depth_T, final_depth_T, rad_confidence_T, rad_depth_T
            _, _, _, depth_est, rad_confidence, radar_depth = model(image, radar, focal)
